...
namespace lebrun.clases.complementos
{
    public class Producto
    {
        ...
        public DataTable cargarProductos()
        {
            ...
            tablaDolar8 = database.fDataTable(sqldolar8);
            tablaeuro8 = database.fDataTable(sqleuro8);

            valorDolar8 = Convert.ToDecimal(tablaDolar8.Rows[0]["histo_valor"].ToString().Replace(",", "."));
            valorEuro8 = Convert.ToDecimal(tablaeuro8.Rows[0]["histo_valor"].ToString().Replace(",", "."));
            ...
            StrSql = "SELECT adminv.inv_codigo AS Codigo,adminv.inv_descri AS Descripcion,adminvmed.ime_undmed AS Unidad, " +
                    ...
                    "CASE " +
                     "   WHEN adminv2.pre_ext = 0 THEN 'No disponible' " +
                     "   ELSE TRUNCATE(adminv2.pre_ext,2) " +
                    " END AS Precio_Dolar,  " +
                    "CASE " +
                     "   WHEN adminv2.pre_ext = 0 THEN 'No disponible' " +
                     "   ELSE TRUNCATE(((adminv2.pre_ext*" + valorDolar8 + ")/" + valorEuro8 + "),2)" +
                    " END AS Precio_Euro,  " +
                    "	CASE WHEN curdate()<cast('2021-10-01' as date) THEN " +
	                   " TRUNCATE(admprecios.pre_precio/1000000,2)   " +
	                   " ELSE " +
                       " TRUNCATE(admprecios.pre_precio*1000000,2)" +
                       " END as Precio_Recon " +
                    "FROM  adminv " +
        ...
        public void ajustarCanInventario(decimal cantidadV, string codigoP, string tipoDocumento, string numDoc, string codCliente)
        {
            ...
            if (tipoDocumento.Equals("FAV"))
            {
                ...
                sentencia2 = sentencia2.Replace("$3", "" + DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day);
        ...
        public void habilitarNuevoPrecio(DataGridView tabla)
        {
            ...
            StrSql = StrSql.Replace("$Sistema", DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day);
      ...
      /* public string Validar_UnidaPrincipal(string codigo)
       {
        ...
        public DataTable cargarProductoVisor3()
        {
            ...
            StrSql = "SELECT adminv.inv_codigo AS Codigo,adminv.inv_descri AS Descripcion,adminvmed.ime_undmed AS Unidad, " +
                    ...
                    "CASE " +
                     ...
                     "   ELSE TRUNCATE(adminv2.pre_ext,2) " +
                    " END AS Precio_Dolar,  " +
                    "'No disponible' as Precio_Petro , 'No disponible' as Precio_Euro " +
                    "FROM  adminv " +
                    "LEFT OUTER JOIN adminv2 ON adminv.inv_codigo = adminv2.inv2_codigo " +
                    "LEFT JOIN adminvmed ON adminvmed.ime_codigo=adminv.inv_codigo " +
                    "LEFT JOIN admprecios ON admprecios.pre_codigo=adminv.inv_codigo AND admprecios.pre_lista='A' AND pre_act='1' " +
        ...
        public void actualizarPrecio_ext(DataGridView tabla)
        {
            StrSql = null;
            for (int i = 0; i < tabla.Rows.Count; i++)
            {
                StrSql = " UPDATE Adminv2 SET pre_ext='$precioDolar' where inv2_codigo='$codigo'";

                StrSql = StrSql.Replace("$codigo", "" + tabla.Rows[i].Cells[2].Value);
                StrSql = StrSql.Replace("$precioDolar", "" + tabla.Rows[i].Cells[13].Value.ToString().Replace(",", "."));
        ...
        public void inhabilitarPrecioMasivo(DataGridView tabla)
        {
            StrSql2 = null;
            StrSql = null;
            DataTable dtExistencia;
            ...
            try
            {
                for (int i = 0; i < tabla.Rows.Count; i++)
                {
                    ...
                    StrSql2 = "SELECT existencia,pre_ext FROM adminv2 WHERE inv2_codigo='$1';";
                    StrSql2 = StrSql2.Replace("$1", "" + tabla.Rows[i].Cells[2].Value);
                    dtExistencia = database.fDataTable(StrSql2);
                    existencia = Convert.ToDecimal(dtExistencia.Rows[0]["existencia"].ToString());
                    precio_ext = Convert.ToDecimal(dtExistencia.Rows[0]["pre_ext"].ToString());

                    if (dtExistencia.Rows.Count > 0)
                    {
                        if ((existencia > 0) && (precio_ext) > 0)
                        {
                            StrSql = " UPDATE admprecios SET pre_act=0 where pre_codigo='$codigo' AND pre_undmed='$undMed' AND pre_lista='$tipoLista' AND pre_codmon='Bs' AND pre_act='1'";
        ...
        public void habilitarNuevoPrecioMasivo(string usuario)
        {
            ...
            StrSql2 = "SELECT inv2_codigo codigo, " +
           " pre_ext*(select histo_valor from admhistodivi where histo_moneda='$8' order by histo_fecha desc, histo_hora desc  limit 1) nuevoPrecio, " +
           " inv_descri descripcion," +
           " ime_undmed unidadMedida," +
           " pre_ext FROM adminv2  inv2" +
           " inner join adminv inv on inv2.inv2_codigo=inv.inv_codigo" +
           " inner join  adminvmed  ume on ume.ime_codigo=inv2.inv2_codigo" +
           " where existencia>0 and pre_ext>0";

            tabla = database.fDataTable(StrSql2);
            ...
            try
            {
                ...
                for (int i = 0; i < tabla.Rows.Count; i++)
                {
                    ...
                    inhabilitarPrecio(tabla.Rows[i]["codigo"].ToString(), "A", tabla.Rows[i]["unidadMedida"].ToString());

                    StrSql = "INSERT INTO admprecios (pre_lista,pre_precio,pre_undmed,pre_codigo, " +
                    "pre_descue,pre_desmax,pre_utilid,pre_fechav,pre_codmon,pre_fecha,pre_act, " +
                    "pre_hora,pre_usuario) VALUES " +
                    "('A','$PrecioNuevo','$Unidad','$Codigo',0,0,0,'$fecha','Bs'," +
                    "'$Sistema','1','$hora','$usuario')";


                    StrSql = StrSql.Replace("$Codigo", "" + tabla.Rows[i]["codigo"].ToString());
                    StrSql = StrSql.Replace("$Descripcion", "" + tabla.Rows[i]["descripcion"].ToString());
                    StrSql = StrSql.Replace("$Unidad", "" + tabla.Rows[i]["unidadMedida"].ToString());
                    StrSql = StrSql.Replace("$PrecioNuevo", "" + tabla.Rows[i]["nuevoPrecio"].ToString().Replace(",","."));
                    StrSql = StrSql.Replace("$fecha", DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day);
                    StrSql = StrSql.Replace("$Sistema", DateTime.Now.Year + "-" + DateTime.Now.Month + "-" + DateTime.Now.Day);
                    StrSql = StrSql.Replace("$hora", DateTime.Now.ToString("hh:mm:ss tt"));
                    StrSql = StrSql.Replace("$usuario", "" + usuario);
        ...
        public void inhabilitarPrecio(string codigo, string tipoLista, string unMed)
        {
            StrSql = null;
            ...
            try
            {
                ...
                StrSql = StrSql.Replace("$codigo", "" + codigo);
                StrSql = StrSql.Replace("$tipoLista", "" + tipoLista);
                StrSql = StrSql.Replace("$undMed", "" + unMed);
                //StrSql = StrSql.Replace("$usuario", "" + tabla.Rows[i].Cells[12].Value);
